...
export class DashboardComponent{
  ...
  constructor(private service: MyHttpService, private datacenter: DataSharingSerivce, private accountService: AccountService){}
  ...
  ngOnInit(){
    if(this.accountService.getIsLoggedIn()) {
      ...
      alert("Please login to see your statistics")
  ...
  showDailyChart(typeOfData: string, year: number, dayOfTheYear: number){
    ...
    this.service.get(environment.apiSecuredUrls+"/dashboard/dataOfDay/"+typeOfData+"/"+year+"/"+dayOfTheYear).subscribe((dataFromBackend)=>{     
  ...
  showWeeklyChart(typeOfData: string, year: number, weekOfTheYear: number){
    ...
    this.service.get(environment.apiSecuredUrls+"/dashboard/dataOfWeek/"+typeOfData+"/"+year+"/"+weekOfTheYear).subscribe((dataFromBackend)=>{     
      this.actualData = dataFromBackend
  ...
  showAnnualChart(typeOfData: string, year: number){
    ...
    this.service.get(environment.apiSecuredUrls+"/dashboard/dataOfYear/"+typeOfData+"/"+year).subscribe((dataFromBackend)=>{     
      this.actualData = dataFromBackend
